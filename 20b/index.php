...
for($t = 0; $t < 25; $t++){
	...
	for($i = 0; $i < count($parts); $i++){
		...
		for($j = 0; $j < count($parts); $j++){
			...
			if(!in_array($j, $dest) && $parts[$i][0] == $parts[$j][0] && $parts[$i][1] == $parts[$j][1] && $parts[$i][2] == $parts[$j][2]){
				if(!in_array($i, $dest)&& !in_array($i, $new_dest)){
					$new_dest[] = $i;
				}
				if(!in_array($j, $dest) && !in_array($j, $new_dest)){
					$new_dest[] = $j;
				}
	...
	$dest = array_merge($dest, $new_dest);
		
}
var_dump($dest);


echo count($parts) - count($dest);

...
$str = 'p=<-833,-499,-1391>, v=<84,17,61>, a=<-4,1,1>
...
for($i = 0; $i < count($parts); $i++){
	for($j = 0; $j < 9; $j++){
		$parts[$i][$j] = str_replace(array('p', '=', '<', '>', 'a', 'v'), '', $parts[$i][$j]);
	}
	$a[] = abs($parts[$i][6]) + abs($parts[$i][7]) + abs($parts[$i][8]);
}
...
for($i = 0; $i < count($parts); $i++){
	...
	if((abs($parts[$i][6]) + abs($parts[$i][7]) + abs($parts[$i][8])) == $min){
		$new_parts[] = $parts[$i];
		echo $i . '<hr>';
	}
...
for($t = 0; $t < 1000; $t++){
	...
	for($i = 0; $i < count($new_parts); $i++){
		...
		$new_parts[$i][3] += $new_parts[$i][6];
		$new_parts[$i][4] += $new_parts[$i][7];
		$new_parts[$i][5] += $new_parts[$i][8];
		
		$new_parts[$i][0] += $new_parts[$i][3];
		$new_parts[$i][1] += $new_parts[$i][4];
		$new_parts[$i][2] += $new_parts[$i][5];

		$pos[$i] = abs($new_parts[$i][0]) + abs($new_parts[$i][1]) + abs($new_parts[$i][2]);
		
	}
	$min = array_search(min($pos), $pos);
